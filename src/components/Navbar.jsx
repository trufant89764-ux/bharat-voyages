...
const Navbar = () => {
  ...
  return <motion.header
    ...
    className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${isHome ? "glass" : "bg-background/95 backdrop-blur-md border-b border-border"}`}
  >
    ...
    const isActive = location.pathname === linkPath && (linkCategory ? currentCategory === linkCategory : linkExclude ? currentExclude === linkExclude && !currentCategory : !currentCategory && !currentExclude);
    return <Link
      key={link.to}
      to={link.to}
      className={`font-body text-sm font-medium transition-colors hover:text-primary ${isActive ? "text-primary" : isHome ? "text-white/90" : "text-muted-foreground"}`}
    >
                {link.label}
              </Link>;
  })}
  ...
  >
            {open ? <X className={isHome ? "text-white" : "text-foreground"} /> : <Menu className={isHome ? "text-white" : "text-foreground"} />}
          </button>
        </div>
      </div>
  ...
  >
    ...
    signOut();
    setOpen(false);
  }} className="font-body text-sm font-medium py-2 text-left text-muted-foreground hover:text-foreground transition-colors">
                    Sign Out
                  </button>
                </> : <Link
    to="/auth"
