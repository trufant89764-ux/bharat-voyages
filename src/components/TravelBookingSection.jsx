...
const TravelBookingSection = () => {
  ...
  const [departDate, setDepartDate] = useState(() => (/* @__PURE__ */ new Date()).toISOString().split("T")[0]);
  ...
  >
    ...
    return <button
      ...
      className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 px-4 py-3 rounded-2xl font-medium text-sm transition-all ${active ? "bg-gradient-to-r from-blue-600 to-pink-500 text-white shadow-lg scale-[1.02]" : "text-foreground/70 hover:bg-white/60 dark:hover:bg-card"}`}
    >
  ...
  >
    ...
    /* Trip type + fare type */
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
                {showTripType ? <div className="inline-flex rounded-full bg-muted p-1">
                    <button
    onClick={() => setTripType("oneway")}
    className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all ${tripType === "oneway" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
  >
                      One Way
                    </button>
                    <button
    onClick={() => setTripType("round")}
    className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all ${tripType === "round" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
  >
                      Round Trip
                    </button>
                  </div> : <span />}

                {showFareTypes && <div className="flex flex-wrap gap-2">
                    {FARE_TYPES.map((f) => <button
    key={f}
    onClick={() => setFareType(f)}
    className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-all ${fareType === f ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border text-muted-foreground hover:text-foreground"}`}
  >
                        {f}
                      </button>)}
                  </div>}
  ...
  />
                ...
                {tripType === "round" && <div className="md:col-span-2">
  ...
  />
                ...
                {showClass && <div className="md:col-span-3">
...
var stdin_default = TravelBookingSection;
export {
