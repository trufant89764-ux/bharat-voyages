...
const CurrencyConverter = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://open.er-api.com/v6/latest/INR").then((r) => r.json()).then((d) => {
      ...
      setRates(next);
      setUpdatedAt((/* @__PURE__ */ new Date()).toLocaleString("en-IN"));
    }).catch(() => {
            ...
            <div>
              ...
              <div className="flex gap-2">
                <div className="flex-1 px-3 py-2 rounded-lg bg-muted/50 border border-border font-semibold">
                  {loading ? <Loader2 size={16} className="animate-spin" /> : result.toLocaleString("en-IN", { maximumFractionDigits: 2 })}
                </div>
                <select
    value={to}
...
var stdin_default = CurrencyConverter;
export {
