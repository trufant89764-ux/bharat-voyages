      ...
      <div className="container-custom">
        ...
        <div onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
          {loading ? <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-80 rounded-2xl bg-muted animate-pulse" />)}
            </div> : <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {visibleCards.map((dest, i) => <Link
    key={`${dest.id}-${i}`}
...
var stdin_default = ExplorePlacesSection;
export {
