...
const AutoCarousel = ({ children, autoplayInterval = 3e3 }) => {
  ...
  const scroll = (dir) => {
    const el = scrollRef.current;
    if (!el) return;
    const cardWidth = el.querySelector(":scope > div")?.offsetWidth || el.clientWidth * 0.3;
  ...
  return <div className="relative group" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div
    ref={scrollRef}
    className="flex gap-6 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-4"
    style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
  >
        {children.map((child, i) => <div key={i} className="snap-start shrink-0 w-[85%] sm:w-[45%] lg:w-[30%]">
            {child}
          </div>)}
      </div>
...
var stdin_default = AutoCarousel;
export {
