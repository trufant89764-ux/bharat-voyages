...
const Destinations = () => {
  ...
  return <div className="pt-20 min-h-screen bg-background">
    ...
    /* Price range filters */
        ...
        {showFilters && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex flex-wrap gap-4 mb-6 p-4 rounded-xl bg-card border border-border">
            <div className="flex items-center gap-2">
              <label className="text-xs text-muted-foreground">Min ₹</label>
              <input type="number" value={priceMin || ""} onChange={(e) => {
    setPriceMin(e.target.value ? Number(e.target.value) : void 0);
    setPage(1);
  }} placeholder="0" className="w-24 px-3 py-2 rounded-lg bg-muted border border-border text-sm text-foreground focus:outline-none" />
            </div>
            <div className="flex items-center gap-2">
              <label className="text-xs text-muted-foreground">Max ₹</label>
              <input type="number" value={priceMax || ""} onChange={(e) => {
    setPriceMax(e.target.value ? Number(e.target.value) : void 0);
    setPage(1);
  }} placeholder="50000" className="w-24 px-3 py-2 rounded-lg bg-muted border border-border text-sm text-foreground focus:outline-none" />
            </div>
            <button onClick={() => {
    setPriceMin(void 0);
    setPriceMax(void 0);
    ...
    /* Categories */
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {categories.map((cat) => <button key={cat.id} onClick={() => handleCategoryChange(cat.name)} className={`px-4 py-2 rounded-full text-xs font-medium transition-colors ${selectedCategory === cat.name ? "gold-gradient text-accent-foreground" : "bg-card border border-border text-muted-foreground hover:text-foreground"}`}>
              {cat.icon} {cat.name}
            </button>)}
        </div>
    ...
    /* Grid */
        ...
        {loading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: PAGE_SIZE }).map((_, i) => <div key={i} className="rounded-2xl bg-muted animate-pulse h-80" />)}
          </div> : destinations.length === 0 ? <p className="text-center text-muted-foreground py-20">No destinations found. Try adjusting your filters.</p> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {destinations.map((dest, i) => <motion.div key={dest.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
    ...
    /* Pagination */
        ...
        {totalPages > 1 && <div className="flex items-center justify-center gap-2 mt-10">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).filter((p) => p === 1 || p === totalPages || Math.abs(p - page) <= 1).map((p, idx, arr) => <span key={p}>
...
var stdin_default = Destinations;
export {
