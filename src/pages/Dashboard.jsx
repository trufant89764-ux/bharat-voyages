...
const Dashboard = () => {
  ...
  const [profile, setProfile] = useState({ name: "", avatar_url: null });
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const { data: bData } = await supabase.from("bookings").select("*, destinations(title, state, image)").eq("user_id", user.id).order("created_at", { ascending: false });
      setBookings(bData || []);
      logger.success("GET", "bookings", bData?.length);
      logger.request("GET", "profiles", { user_id: user.id });
      const { data: pData } = await supabase.from("profiles").select("name, avatar_url").eq("user_id", user.id).single();
  ...
  const handleProfileSave = async () => {
    ...
    const { error } = await supabase.from("profiles").update({ name: profile.name, avatar_url: profile.avatar_url }).eq("user_id", user.id);
    if (error) {
  ...
  return <div className="pt-20 min-h-screen bg-background">
    ...
    /* Tabs */
        ...
        <div className="flex gap-2 mb-6">
          {["bookings", "profile"].map((t) => <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${tab === t ? "gold-gradient text-accent-foreground" : "bg-card border border-border text-muted-foreground hover:text-foreground"}`}>
              {t === "bookings" ? "My Bookings" : "Edit Profile"}
            </button>)}
        </div>

        {tab === "profile" && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 p-6 rounded-2xl bg-card border border-border">
        ...
        {tab === "bookings" && <>
            {loading ? <div className="space-y-4">
              ...
              </div> : bookings.length === 0 ? <div className="text-center py-16">
                <p className="text-muted-foreground mb-4">No bookings yet</p>
                <Link to="/destinations" className="text-primary font-medium hover:underline">Explore destinations →</Link>
              </div> : <div className="space-y-4">
                {bookings.map((b) => <motion.div key={b.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-4 p-4 rounded-xl bg-card border border-border">
                    {b.destinations?.image && <img src={b.destinations.image} alt={b.destinations.title} className="w-20 h-20 rounded-lg object-cover shrink-0" />}
