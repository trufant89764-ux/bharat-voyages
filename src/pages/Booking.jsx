...
const Booking = () => {
  ...
  const handleConfirm = async () => {
    ...
    setSubmitting(true);
    const { data: dbDest } = await supabase.from("destinations").select("id").eq("title", activeDest.title).maybeSingle();
  ...
  return <div className="pt-20 min-h-screen bg-background">
      <div className="container-custom py-10 max-w-2xl">
        ...
        {!user && <div className="mb-6 p-4 rounded-xl bg-primary/10 border border-primary/20 text-sm text-foreground">
        ...
        {mode && MODE_LABELS[mode] && <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-blue-600/10 to-pink-500/10 border border-primary/20 text-sm text-foreground">
            <strong className="text-primary">{MODE_LABELS[mode]} search:</strong>{" "}
            {fromCity || "\u2014"} → {toCity || "\u2014"}
            {queryDate ? ` on ${queryDate}` : ""}
            {queryTravellers ? ` \xB7 ${queryTravellers} traveller${Number(queryTravellers) > 1 ? "s" : ""}` : ""}
        ...
        {step === 1 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div>
              ...
              <select value={selectedDest} onChange={(e) => setSelectedDest(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-card border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                ...
                {destinations.map((d) => <option key={d.id} value={d.id}>{d.title} — ₹{d.price.toLocaleString("en-IN")}/person</option>)}
              </select>
            </div>
            <div>
            ...
            {activeDest && <div className="p-4 rounded-xl bg-muted">
        ...
        {step === 2 && activeDest && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        ...
        {step === 3 && activeDest && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="p-6 rounded-2xl bg-card border border-border space-y-4">
